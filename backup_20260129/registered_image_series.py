...
class RegisteredImageSeries:
    ...
    def add(self,image,x=None,y=None,z=None):
    ...
    #     """
    #     Build corrected images on a common canvas.
    #     Assumes each image has shape (sy, sz, sx) = (Y, Z, X).
    ...
    #     assert len(self.images) > 0, "No images added."
    ...
    #     # --- iterate each image with its shift maps ---
    #     for idx, (vol, xmap, ymap, zmap) in enumerate(zip(self.images, xmaps, ymaps, zmaps)):
    #         print(f"[RVS] Correcting image {idx} on canvas "
    ...
    def correct_images(self):
        # Determine how large the rendered image must be.
        ...
        #    and add this to the original image dimensions
        ...
        counter = 0
        for vol, x_shift_map, y_shift_map, z_shift_map in zip(self.images, self.xshifts, self.yshifts, self.zshifts):
            
            corrected_image = np.full((expanded_y, expanded_z, expanded_x), np.nan, dtype=complex)
            print(f"Correcting image {counter}")
            ...
            for y in range(sy):
                for x in range(sx):
                    ...
                    corrected_image[yy, zz:zz+sz, xx] = ascan
                    # corrected_image[yy, :sz, xx] = ascan
                    print(f"y = {y}, x = {x}")
            counter += 1      
    ...
    def _rfunc():
        ...
        try:
            ...
            raise ImportError(
                ...
                "Make sure it sits next to ucd_registered_image_series.py."
            ) from e

    # ---- loaders / cropping ----

    @classmethod
    def load_image_from_folder(cls, folder: str, prefix: str = "bscan", *, crop: bool = False):
        """
        Wrapper over rfunc.get_image / get_image_and_crop (returns shape (slow, depth, fast)).
        """
        rfunc = cls._rfunc()
        vol = rfunc.get_image(folder, prefix=prefix)            # (slow, depth, fast)
        return vol

    @classmethod
    def auto_crop_image(cls, image: np.ndarray):
        """
        Ask rfunc.auto_crop for z1/z2 based on z-profile, then slice.
        Expects (slow, depth, fast); returns cropped image with same ordering.
        """
        rfunc = cls._rfunc()
        z1, z2 = rfunc.auto_crop(image)
        return image[:, z1:z2, :], (int(z1), int(z2))

    # ---- flattening (handles axis order differences automatically) ----

    @classmethod
    def flatten_image_x(cls, image_yzx: np.ndarray):
        """
        rfunc.flatten_image expects (slow, fast, depth).
        Your pipeline uses (slow, depth, fast). We transpose to match,
        call rfunc, then transpose back.
        """
        rfunc = cls._rfunc()
        vol_yxz = np.transpose(image_yzx, (0, 2, 1))        # (slow, fast, depth)
        flat_yxz = rfunc.flatten_image(vol_yxz)              # internal shear-based x-flatten
    ...
    @staticmethod
    def upsample_image(image: np.ndarray, factor: int) -> np.ndarray:
        import registration_functions as rfunc
        return rfunc.upsample(image, factor)

    @staticmethod
    def project3(image: np.ndarray, pfunc=np.nanmean):
        import registration_functions as rfunc
        return rfunc.project3(image, pfunc=pfunc)

    @staticmethod
    def flythrough3(image: np.ndarray, fps: int = 5):
        import registration_functions as rfunc
        return rfunc.flythrough3(image, fps=fps)
    ...
    class _BroadcastReference:
        """
        Mirrors testing_broadcasting.ReferenceImage:
        - caches 3D FFT of the reference image
    ...
    def register_by_broadcasting(
        ...
        *,
        ...
        Parameters
        ----------
        ref_image : (n_slow, n_depth, n_fast) ndarray
            Reference image.
        target_images : list[(n_slow, n_depth, n_fast) ndarray]
            Target images to register to the reference.
        poxc : bool
            Use phase-only cross-correlation normalization (matches script default).
        plot : bool
            Reproduce the four time-series plots (xc, dy, dz, dx).
        save_dir : str | None
            If provided, save xc arrays and x/y/z maps per target image as .npy.
        ...
        bref = cls._BroadcastReference(ref_image)
        ...
        # 8) Correct images through the class’ existing API
        rvs.correct_images()

        return dict(
            rvs=rvs,
            per_image=per_image_results,
            corrected_images=getattr(rvs, "corrected_images", []),
        )
    ...
    def register_by_broadcasting_from_root(
        ...
        *,
        prefix: str = "bscan_",  # keep flexible; original used rfunc.get_image_and_crop
        loader=None,             # optional callable: (folder) -> image ndarray
        **kwargs
    ):
        """
        Convenience: load images from numbered subfolders under `root`,
        then call `register_by_broadcasting`. Keep behavior flexible; you can
        pass a custom `loader` that replicates `rfunc.get_image_and_crop`.
        """
    ...
    def build_and_register_from_root(
        ...
        **broadcast_kwargs,
        ...
        Steps:
        1) Load images (optionally auto-crop at load-time).
        ...
        5) Correct images with RVS API; return a structured dict.
        ...
        z_crop = None
        for f in folders:
            vol = cls.load_image_from_folder(f, prefix=prefix, crop=False)  # (slow, depth, fast)
            if crop and z_crop is None:
                # Record the first crop window for reference
                vol, z_crop = cls.auto_crop_image(vol)
            elif crop:
            ...
            if flatten_x:
                vol = cls.flatten_image_x(vol)
    
            if upsample_factor and upsample_factor > 1:
                vol = cls.upsample_image(vol, upsample_factor)
        ...
        if use_broadcasting:
            reg = cls.register_by_broadcasting(
                ref_image=vols[0],
                target_images=vols,        # like your script: include ref first
                plot=plot,
                save_dir=save_dir,
                **broadcast_kwargs
            )
            ...
            rvs.correct_images()
            reg = {"rvs": rvs, "per_image": [], "corrected_images": getattr(rvs, "corrected_images", [])}
